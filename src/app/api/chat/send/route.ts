...
- 当 should_generate_voice 为 true 时，messages[0] 必须是角色正常的聊天回复文字（比如"给你录一段"）。系统会自动把 messages[0] 转成语音发送给用户。以下行为绝对禁止：
  ① 禁止在 messages[0] 里写 "[语音]"、"[语音消息]" 等占位符。
  ② 禁止在 messages[0] 里写动作描写或场景描述（如"（脚步声有点重）"、"（笑）"、"（沉默）"、"（看了下时间）"），这些只能写在 scene_description 里。
  ③ 禁止在 messages[0] 里用换行（\n\n）追加额外内容，messages[0] 必须是一段完整的纯对话文字，像真人发微信一样自然简短。
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (shouldGenerateVoice) {
      ...
      // 优先找有效消息（非占位符且清理后有内容）
      const validVoiceText = llmResult.messages.find((m) => {
        const cleaned = cleanVoiceContent(m);
        return cleaned.length > 3; // 至少3个字符才算有效
      });
      voiceText = validVoiceText ? cleanVoiceContent(validVoiceText) : "";

      // 如果过滤后没有有效内容，不生成语音
      if (!voiceText) {
        console.log("[Send] voice content empty after cleaning, skip voice generation");
      } else {
